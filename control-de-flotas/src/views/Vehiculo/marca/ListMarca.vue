      ...
      <v-dialog v-model="dialog2" fullscreen hide-overlay transition="dialog-bottom-transition">
        <v-card>
          <v-toolbar dark color="primary">
            ...
            <v-toolbar-title>Editar Marca</v-toolbar-title>
            <v-spacer></v-spacer>
            <v-toolbar-items>
...
export default {
  ...
  methods: {
    ...
    editarRegistro(){
        this.$store.dispatch("updateMarca", this.edit).then(() => {
            // this.$store.dispatch("loadMarcasActivas");
            this.dialog2 = false;
    ...
    onCheckboxChange(newValue, oldValue) {
      if (newValue == true) {
        this.$store.dispatch("loadMarcasInActivas")
      }else{
        this.$store.dispatch("loadMarcasActivas")
      }
    },
